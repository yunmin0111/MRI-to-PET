"""
Train the 2D VQ-VAE (offset-attention) for MRI -> PET slice translation.

- INPUT  : MRI slice
- TARGET : paired PET slice
- LOSS   : distance(recon, PET) + VQ loss
           --dist l1  ->  L1  (default)
           --dist l2  ->  MSE
  (Spec: "simple distance difference for loss".)

No perceptual/adversarial terms here (kept intentionally simple per spec).
The autoencoder thus learns MRI->PET directly; its encoder latent will feed the
LDM/DDPM diffusion stage later.
"""
...
from models.vqvae_2d_offset import VQVAE2DOffset
...
def main():
    ...
    root = cfg['data']['root']
    dist = mc.get('dist', 'l1')
    epochs = mc.get('epochs', 100)
    bs = cfg['data'].get('batch_size', 32)

    model = VQVAE2DOffset(
        in_ch=1, out_ch=1,
        base=mc.get('base_channels', 64),
        z_ch=mc.get('embedding_dim', 4),
        num_embeddings=mc.get('num_embeddings', 1024),
        commitment_cost=mc.get('commitment_cost', 0.25),
    ).to(device)
    print(f"VQVAE2DOffset params: {sum(p.numel() for p in model.parameters())/1e6:.2f}M | dist={dist}")
    ...
    ckpt_dir = f"results/{mc['model_name']}/checkpoint"
    ...
    for ep in range(epochs):
        ...
        for mri, pet, _, _ in tr:
            ...
            recon, vq_loss, idx = model(mri)          # INPUT=MRI
            if dist == 'l2':
                rec_loss = F.mse_loss(recon, pet)     # TARGET=PET
            else:
                rec_loss = F.l1_loss(recon, pet)
            loss = rec_loss + vq_loss
            opt.zero_grad(); loss.backward(); opt.step()
            run['rec'] += rec_loss.item(); run['vq'] += vq_loss.item()
            active.update(idx[0].detach().cpu().numpy().tolist())
        n = len(tr)
        usage = len(active) / model.vq.num_embeddings * 100
        ...
        if (ep+1) % 5 == 0:
            ...
            with torch.no_grad():
                for mri, pet, _, _ in va:
                    mri, pet = mri.to(device), pet.to(device)
                    recon, _, _ = model(mri)
                    vr += F.l1_loss(recon, pet).item()
                    vp += psnr(F.mse_loss(recon, pet).item()); m += 1
